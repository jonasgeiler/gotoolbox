...
function makeGithubStub(toolOwner: string, toolRepo: string, toolRelease: any) {
  ...
  return {
    ...
    rest: {
      repos: {
        getReleaseByTag: async ({ owner, repo, tag }: { owner: string; repo: string; tag: string; }) => {
          if (owner !== toolOwner || repo !== toolRepo || tag !== toolRelease.tag_name) {
            throw new RequestError(`No stubbed release for ${owner}/${repo}@${tag}`, 404, {
							request: {
...
describe("release-tools github script", () => {
	it(
		"should work for the dprint tool, which doesn't use a 'v' prefix",
		async () => {
	...
	it(
		"should work for the goreleaser tool, which uses a 'v' prefix",
		async () => {
			process.env.CHANGED_VERSION_GO_FILE_PATHS =
	...
	it(
		"should work for the golangci-lint tool, which uses a 'v' prefix and a html release notes body",
		async () => {
			process.env.CHANGED_VERSION_GO_FILE_PATHS =
			...
			assert.match(
				createdRelease.body,
				/\bgolangci\/golangci-lint@v2\.12\.2\b/
			);
			assert.match(createdRelease.body, /\bgolangci-lint v2\.12\.2\b/);
			assert.match(
				createdRelease.body,
				/<b>golangci-lint<\/b> release notes\b/
			);
			assert.match(
				createdRelease.body,
